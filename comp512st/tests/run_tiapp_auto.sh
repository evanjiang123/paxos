...
# Using trot-open-09 as the test host (all players run on same host with different ports)
export autotesthost=10.69.38.159
...
if [[ ! $autotesthost =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] && [[ $autotesthost != $current_hostname ]]
then
	echo "Error !! This script is configured to run from $autotesthost, but you are trying to run this script on $current_hostname."
	echo "Note: If using IP addresses, this check is skipped. Current host: $current_hostname"
	exit 10
fi
